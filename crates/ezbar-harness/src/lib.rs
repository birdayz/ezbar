...
pub fn run_themed(modules: Vec<Box<dyn Module>>, theme: ThemeTokens) -> iced::Result {
    ...
    iced::application(
        move || {
            Harness::new(
                pending.lock().unwrap().take().expect("boot runs once"),
                theme,
            )
        },
        Harness::update,
...
impl Harness {
    ...
    fn view(&self) -> Element<'_, HMsg> {
        ...
        // ---- background swatches ----
        let mut swatches: Vec<Element<HMsg>> = vec![text("bar background:").color(dim).into()];
        for bg in BarBg::ALL {
        ...
        let popup: Element<HMsg> = match self.popup {
            Some((instance, _mode)) => {
                match self
                    .modules
                    .iter()
                    .find(|e| e.instance == instance && !e.panicked)
                {
                    Some(entry) => {
                        let ctx = Ctx {
                            instance_id: instance,
                        ...
                        match entry.module.popup(&ctx) {
                            Some(content) => {
                                wrap_popup(content.map(move |m| HMsg::Module { instance, msg: m }))
                            }
                            None => hint(&format!(
