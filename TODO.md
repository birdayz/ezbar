...
- [x] **MED** — **Inline markup — DONE (both modules).** The themed `[c=token]…[/c]` /
  `[b]…[/b]` subset (RFC 0002) is a renderer-agnostic `modules::markup` (parse → coalesce →
  ...
  — the markup is parsed ONCE off the trusted format and the title substituted AFTER, so a
