...
fn cfg_dir() -> Option<String> {
    std::env::var("HOME").ok().map(|h| format!("{h}/.config/ezbar"))
}
...
async fn load_access_token() -> Auth {
    ...
    if let Some(mut tok) = read_token() {
        ...
        if !tok.refresh_token.is_empty() {
            if refresh_token(&mut tok).await.is_ok() {
                return Auth::Token(tok.access_token);
            }
        }
    }
...
fn basic_auth(cfg: &SpotifyConfig) -> String {
    base64::engine::general_purpose::STANDARD.encode(format!("{}:{}", cfg.client_id, cfg.client_secret))
}
...
async fn get_current_track(token: &str) -> Result<SpotifyData, SpErr> {
    ...
    match resp.status().as_u16() {
        ...
        200 => {
            ...
            let artist = v["item"]["artists"][0]["name"].as_str().unwrap_or("").to_string();
            let album = v["item"]["album"]["name"].as_str().unwrap_or("").to_string();
            let is_playing = v["is_playing"].as_bool().unwrap_or(false);
            let icon = if is_playing { "🎵" } else { "⏸️" };
...
fn open_browser(url: &str) {
    ...
    log::info!("open this URL to authorize Spotify: {url}");
}
