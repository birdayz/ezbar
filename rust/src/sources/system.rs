...
pub fn get_cpu_usage() -> String {
    ...
    format!("🖥️ {}%", cpu_usage_percent(&cpu1, &cpu2))
}
...
pub fn get_cpu_temperature() -> String {
    match read_cpu_temp_millic() {
        Some(mc) => format!("🌡️ {:.0}°C", mc as f64 / 1000.0),
        None => "🌡️ --".to_string(),
    }
}
    ...
    if let Ok(entries) = fs::read_dir("/sys/class/thermal") {
        for dir in entries.flatten().map(|e| e.path()) {
            ...
            if t == "x86_pkg_temp" || t == "cpu-thermal" || t.contains("cpu") {
                if let Ok(mc) = fs::read_to_string(dir.join("temp")).unwrap_or_default().trim().parse() {
                    return Some(mc);
                }
            }
        }
...
fn read_temp_inputs(dir: &std::path::Path) -> Vec<(String, i64)> {
    ...
    if let Ok(entries) = fs::read_dir(dir) {
        ...
        inputs.sort();
        for input in inputs {
            let Some(value) = fs::read_to_string(&input).ok().and_then(|s| s.trim().parse::<i64>().ok())
            else {
                continue;
            };
            let label_name = input.file_name().unwrap().to_string_lossy().replace("_input", "_label");
...
mod tests {
    ...
    fn extract_values_from_display_strings() {
        assert_eq!(extract_cpu_usage_value("🖥️ 45%"), 45.0);
        assert_eq!(extract_cpu_usage_value("🖥️ 0%"), 0.0);
        assert_eq!(extract_cpu_usage_value("🖥️ --"), 0.0);

        assert_eq!(extract_temperature_value("🌡️ 45°C"), 45.0);
        assert_eq!(extract_temperature_value("🌡️ --"), 0.0);
    ...
    fn pick_cpu_temp_prefers_package_sensor() {
        ...
        assert_eq!(pick_cpu_temp(&[("".into(), 50000), ("".into(), 48000)]), Some(50000));
