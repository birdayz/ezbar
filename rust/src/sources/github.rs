...
impl GitHub {
    ...
    pub async fn fetch(&mut self) -> Result<FetchResult, String> {
        ...
        loop {
            ...
            let resp = req.send().await.map_err(|e| format!("listing notifications: {e}"))?;
            ...
            if let Some(arr) = body.as_array() {
                for n in arr {
                    ...
                    all.push(GitHubNotification {
                        ...
                        type_,
                        repo_name: n["repository"]["full_name"].as_str().unwrap_or("").to_string(),
                        updated_at,
    ...
    async fn prune_merged_prs(
        ...
        let checks = ns.iter().enumerate().map(|(i, n)| {
            ...
            let caps = if is_pr_review { re.captures(&n.html_url) } else { None };
