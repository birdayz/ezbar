...
pub fn sway_stream() -> impl Stream<Item = SwayUpdate> {
    stream::channel(50, |mut output: iced::futures::channel::mpsc::Sender<SwayUpdate>| async move {
...
fn sort_workspaces(ws: &mut [Workspace]) {
    ws.sort_by(|a, b| match (a.name.parse::<i32>(), b.name.parse::<i32>()) {
        (Ok(x), Ok(y)) => x.cmp(&y),
        _ => a.name.cmp(&b.name),
    });
}
...
fn run_workspaces(tx: &tokio::sync::mpsc::Sender<SwayUpdate>) -> swayipc::Fallible<()> {
    ...
    for event in events {
        ...
        sort_workspaces(&mut state);
        if tx.blocking_send(SwayUpdate::Workspaces(state.clone())).is_err() {
            break;
        }
    }
    Ok(())
