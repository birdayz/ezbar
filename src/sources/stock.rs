...
fn format_stock(symbol: &str, price: f64, change: f64, change_percent: f64) -> StockData {
    ...
    let display_text = format!(
        "{} {}: {} {}",
        trend_emoji, symbol, price_string, change_string
    );
    StockData {
...
pub async fn fetch(symbol: &str, api_key: &str) -> Result<StockData, String> {
    ...
    Ok(StockData {
        symbol: symbol.to_string(),
        display_text: format!("󰔵 {}: Error fetching data", symbol),
...
async fn fetch_finnhub(symbol: &str, api_key: &str) -> Result<StockData, String> {
    ...
    Ok(format_stock(
        symbol,
        c,
        v["d"].as_f64().unwrap_or(0.0),
        v["dp"].as_f64().unwrap_or(0.0),
    ))
}
...
async fn fetch_alphavantage(symbol: &str, api_key: &str) -> Result<StockData, String> {
    ...
    let price = q["05. price"]
        .as_str()
        .unwrap_or("0")
        .parse::<f64>()
        .unwrap_or(0.0);
    let change = q["09. change"]
        .as_str()
        .unwrap_or("0")
        .parse::<f64>()
        .unwrap_or(0.0);
...
mod tests {
    ...
    fn format_positive() {
        ...
        assert_eq!(d.trend_emoji, "󰔵");
        assert_eq!(d.price_string, "$100.00");
        assert_eq!(d.change_string, "+5.00 (5.26%)");
        assert_eq!(d.display_text, "󰔵 NQ=F: $100.00 +5.00 (5.26%)");
    }
    ...
    fn format_negative() {
        ...
        assert!(!d.is_positive && d.is_negative);
        assert_eq!(d.trend_emoji, "󰔳");
        assert_eq!(d.change_string, "-2.50 (-4.76%)");
        assert_eq!(d.display_text, "󰔳 AAPL: $50.00 -2.50 (-4.76%)");
    }
