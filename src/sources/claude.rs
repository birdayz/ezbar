    ...
    if let Ok(entries) = fs::read_dir("/proc") {
        for e in entries.flatten() {
            ...
            match comm.trim() {
                ...
                "notify-send" if read_cmdline(&pid).contains("waiting for your input") => {
...
fn parse_limits(v: &Value) -> Option<Limits> {
    ...
    Some(Limits {
        ...
        five_h_reset: rl["five_hour"]["resets_at"]
            .as_i64()
            .map(unix_hm)
            .unwrap_or_default(),
        weekly_left: week_used.map(|u| 100.0 - u),
        weekly_reset: rl["seven_day"]["resets_at"]
            .as_i64()
            .map(unix_date)
            .unwrap_or_default(),
    })
