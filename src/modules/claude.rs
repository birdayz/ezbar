...
impl Module for Claude {
    ...
    fn popup(&self, _ctx: &Ctx) -> Option<Element<'_, ModMsg>> {
        ...
        let mut col: Vec<Element<ModMsg>> =
            vec![
                text(format!("Claude — {} instance(s)", self.instances.len()))
                    .size(15)
                    .into(),
            ];
        ...
        for i in &self.instances {
            ...
            col.push(
                row(vec![
                    text(marker).into(),
                    text(i.project.clone()).color(color).into(),
                ])
                .spacing(8)
                .into(),
            );
...
fn limits_stream(_id: &u64) -> impl Stream<Item = ModMsg> {
    ezbar_plugin::iced::stream::channel(
        ...
        |mut out: ezbar_plugin::iced::futures::channel::mpsc::Sender<ModMsg>| async move {
            loop {
                let l = tokio::task::spawn_blocking(claude::limits)
                    .await
                    .ok()
                    .flatten();
